...
import {url as LyricUrl,ERR_OK} from "@/api/getLyric";
...
export default {
  ...
  methods:{
    ...
    _getLyric(){
      let songmid=this.currentSong.mid;
      var lyrurl=LyricUrl+"&songmid="+songmid;
      if(this.currentSong.mid){
      axios(lyrurl).then(response=>{
    ...
    MiddleTouchMove(e){
      ...
      const deltaX=e.touches[0].pageX-this.touch.startX;//为cd时不能右滑，为歌词时不能左滑
